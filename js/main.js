...
async function render(tab) {
    ...
    try {
        ...
        app.innerHTML = `<div class="text-red-400 text-center py-8">Error loading content: ${error.message}</div>`;
...
window.openCategory = async function(cat) {
    ...
    try {
        ...
        app.innerHTML = `
            <button class="btn mb-6" onclick="history.back()">← Back</button>
        ...
        history.pushState({ category: cat }, '', `#category-${encodeURIComponent(cat)}`);
    } catch (error) {
        app.innerHTML = `<div class="text-red-400 text-center py-8">Error loading challenges: ${error.message}</div>`;
